...
C-y : paste.
q : quit.
* Magit — Resumo, Instalação e Uso Básico
O Magit é uma interface avançada para Git integrada ao Emacs.
Ele permite realizar praticamente todas as operações de versionamento
sem sair do editor, com forte ênfase em clareza, segurança e controle.

O Magit não oculta o Git; ao contrário, expõe explicitamente o estado do
repositório e incentiva boas práticas de versionamento.
...
Configurações simples que melhoram a experiência inicial:

#+begin_src emacs-lisp
;; Atalho global para o status do Magit
(global-set-key (kbd "C-x g") 'magit-status)

;; Evita confirmações excessivas em operações comuns
...
O fluxo de trabalho no Magit segue explicitamente o modelo do Git:
- *Working Tree*
- *Staging Area*
- *Repository*

O ponto de entrada principal é:
- =C-x g= → abre o buffer *magit-status*

** Operações básicas no buffer magit-status

A partir do buffer de status:

- =g= → atualizar o status
- =s= → adicionar (stage) alteração sob o cursor
- =u= → remover (unstage) alteração
- =S= → adicionar todas as alterações
- =c c= → criar commit
- =P p= → push para repositório remoto
- =F p= → pull do repositório remoto
O Magit sempre solicita confirmação explícita antes de operações destrutivas.

** Commits
Fluxo típico de commit:
1. Selecionar alterações desejadas (=s=).
2. Iniciar commit com =c c=.
3. Escrever mensagem de commit no buffer apropriado.
4. Confirmar o commit.
Esse processo reforça o conceito de commits atômicos e bem documentados.

** Integração com GitHub
O Magit funciona diretamente com repositórios hospedados no GitHub:
- Push e pull via HTTPS ou SSH.
- Suporte a autenticação moderna (tokens ou chaves SSH).
- Integração natural com fluxos baseados em *pull requests*.

Para uso acadêmico, o Magit permite que todo o ciclo de versionamento
ocorra dentro do Emacs, reduzindo troca de contexto e erros operacionais.
...
* [revisando os comandos - PAREI AQUI]
- Testar o comando.
- Se tiver dúvida comparar com outros guias.** Expressões
...
(assoc 'Molly map_nome_raca)
-> (Molly "Border Collie")

** Manipulando Listas
*** cons: É usado para contruir listas adicionando elementos; cons recebe como parâmetros a lista e o próximo elemento.
(setq x 1) -> 1
(setq y 2) -> 2
(setq z 3) -> 3
...
O primeiroElemento pode ser obtido com o comando car.
(car par) -> 1
O segundoElemento é obtido com o comando cdr.
(cdr par) -> 2
...
*** car: Extrai o primeiro elemento da lista (c++ front)
(car '(Molly Bueno)) -> Molly
(car doguinhos) -> Molly

*** cdr: Elimina o primeiro elemento da lista
(Equivalente C++: pop_front)
(cdr '(Molly da Luz Bueno)) -> da Luz Bueno
(car doguinhos) -> Molly
(print doguinhos) -> (Molly Nina Koto Faisca)

*** nth: Extrai o elemento 3 da lista, o índice começa em 0 (c++ v[n])
(setq doguinhos '(Molly Nina Koto Faisca))
...
(Equivalente C++: for_each(l.begin(),l.end(),f);)
...
(vconcat [0 1 2 3] [?a ?b ?c ?d ?e] ["abcde"] ) -> [0 1 2 3 97 98 99 100 101 "abcde"]
...
(vectorp ( make−bool−vector 3 nil )) -> nil : um vetor de boleanos não é um vector.
...
*** aref: Usada para acessar elementos de um vetor. Note que aref é array reference.
(aref "abcdefg" 0) -> 97 ; a=97
(aref "abcdefg" 2) -> 98 ; b=98
(aref "abcdefg" 3) -> 99 ; c=99
...
Note que o arquivo tem 64015 caracteres e estamos na posição 32604.
(point) -> 32604
(point-min) -> 1 (início do buffer).
(point-max) -> 66052 (fim do buffer).
...
(fset 'multiplica (lambda (x y) (* x y)))
(multiplica 3 4) -> 12
...
(print element))
*** dotimes: Repete n vezes (equivale a um for de C/C++)
...
** TODO Corrigir aqui e depois copiar para lyx;
** TODO Converter esta parte em elisp-guiareferencia-bueno.org
...
alias emacs29="emacs --init-directory=~/.emacs.d-v29/"
...
Adicionar atalho para: M-x highlight-changes-mode : Mostra o que mudou a partir da execução do comando, mudanças aparecem em vermelho.
